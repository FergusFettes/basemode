...
class _TreeEntry:
    root: Node
    node_count: int
    root_preview: str   # truncated first-paragraph text
...
class TreePickerView(Widget):
    ...
    def load(self, store: GenerationStore, current_root_id: str) -> None:
        ...
        for root in roots:
            ...
            root_preview = self._shorten(root.text)

            last_id = root.metadata.get("last_node_id")
            if last_id and last_id != root.id:
                node = store.get(last_id)
                leaf_preview = self._shorten(node.text) if node else _NONE_LABEL
            else:
    ...
    def render(self) -> Text:
        ...
        for i, entry in enumerate(self._entries):
            ...
            gap = max(2, width - len(header_left) - len(count_str))
            header = header_left + " " * gap + count_str
